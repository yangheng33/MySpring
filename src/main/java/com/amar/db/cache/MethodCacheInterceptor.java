...
public class MethodCacheInterceptor implements MethodInterceptor, InitializingBean
{
	...
	{
		// 这个表示哪个类调用（或触发）了这个MethodCacheInterceptor，如里的:manager.PersonMagagerImpl
		String targetName = invocation.getThis().getClass().getName();

		// 这个表示哪个方法触发了这个类（MethodCacheInterceptor）方法（invoke）的调用，如这里的:getList
		...
		if ( element == null )
		{
			...
			/*
			 * 这个就是调用数据访问方法，如这里是调用manager.PersonManagerImpl.getList(),
			 * 并用result保存执行的结果（数据访问的结果）,如这里调用了getList()方法，会先打印出"get Person from DB"
			...
			/*
			 * cache method result
			 * 下面方法执行后，将cacheKey与数据集连起来，cacheKey是用来标识这个element的标志，
			 * 我们可以有多个element(各自是来自不同的数据访问方法而形成的)，区分它们就是用cacheKey
			 */

			element = new Element( cacheKey , ( Serializable ) result );
			/*
			 * 这里的新生成后的element，含有cacheKey，还在element创建时间，访问时间，
			 * 还有命令次数等cache的属性，我觉得它就像是一个小cache一样，下次要不要更新它就要看它的这些属性来决定。
			 */
			// 放入cache中
			cache.put( element );
		}
