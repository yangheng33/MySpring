...
<html>
  ...
  function initFunc()
  {
	  $("#cycle").val("${remindplan.cycle}");
	  $("#type").val("${remindplan.type}");
	  $("#state").val("${remindplan.state}");
	  $("#title").val("${remindplan.title}");
	  $("#cycleparam").val("${remindplan.cycleparam}");
  ...
  function initCheckBox()
  {
	  ...
	  var cycleparams = $("#cycleparam").val();
	  
	  for(var i=0;i<TypeID.length;i++)
	  {
		  
		  if( cycleparams.indexOf(","+TypeID[i].value+"," ) > -1 )
		  {
			  TypeID[i].checked = true;
		  }
      }
  }
  ...
  <body>
   修改提醒<br>
     <form action="<%=path%>/remindplan.amar?method=edit" method="post">
     ...
     <div id="timeDiv" >  
     运行时间:<input type="text" id="exectimes" name="exectimes" readonly="readonly" value="${exectimes}"
