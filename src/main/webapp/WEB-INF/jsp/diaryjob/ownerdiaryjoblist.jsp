...
<html>
  ...
  <body class="commonFont">
	...
	<table cellspacing="1" style="width:800" class="tablesorter">
			<thead>
				<tr>
						<th colspan="7" align="center">${user.realname}</th>
				</tr>
			</thead>
			<tbody align="center">
			<c:forEach items="${userlist}" var="user" >
					...
					<c:if test="${status.index%7==0 }">
						...
						<c:if test="${userDate == '0'}">
							<td  style="background:red" onclick="askAddDiaryjob('${status.count}')">
							未记录${datelist[status.index]}
							</td>
						</c:if>
						<c:if test="${userDate == '1'}">
							<td  style="background:green" onclick="query4Div('${status.count}','${user.userid}')">
							已记录
